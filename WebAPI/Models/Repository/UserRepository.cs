...
namespace WebAPI.Models.Repository
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<Response> RegisterUserAsync(Registration registrationModel)
        {
            var user = await appDbContext.Users.Where(u => u.Email.ToLower().Equals(registrationModel.Email.ToLower())).FirstOrDefaultAsync();
            if (user != null)
                return (new Response() { Success = false, Message = "Email already exist" });
        ...
        public async Task<Response> LoginUserAsync(Login loginModel)
        {
            if (loginModel == null)
        ...
        private string GenerateJwtToken(long id, string name, string email)
        {
